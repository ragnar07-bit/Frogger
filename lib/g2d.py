...
def _get_font(size: int) -> pg.font.Font:
    global _font_name
    if _font_name is None:
        fonts = pg.font.get_fonts()
        _font_name = "segoeuisymbol" if "segoeuisymbol" in fonts else "freesansbold"

    if size not in _font_cache:
        _font_cache[size] = pg.font.SysFont(_font_name, int(size))
